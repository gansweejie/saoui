...
var SAOMainItem = {
  _new: function (name) {
    ...
    var dom = $(
      '<div class="menu_btn icon_box" id="sao_menu_btn_' +
        name +
        '">\
						<div id="' +
        id +
    ...
    var panelContent = { title: "", main_image: "", desc: "" };
    ...
    item.setPanelContent = function (title, image, desc) {
      panelContent.title = title;
      panelContent.main_image = image;
      panelContent.desc = desc;
    };
    item.setPosX = function (value) {
      posX = value;
    };
    item.setPosY = function (value) {
      posY = value;
    };

    item.getDOM = function () {
      return dom;
    };
    ...
    item.getWID = function () {
      return wid;
    };
    item.getPosX = function () {
      return posX;
    };
    item.getPosY = function () {
      return posY;
    };
    ...
    item.setSelected = function () {
      ...
      $("." + class_).css(
        "background",
        "url('" + image_icon_hover + "') no-repeat"
      );
    ...
    item.cancelSelected = function () {
      ...
      $("." + class_).css(
        "background",
        "url('" + image_icon_normal + "') no-repeat"
      );
      $("." + class_).css("background-position", "8px 8px");
    };
    return item;
...
var SAOMainPanel = {
  _new: function () {
      ...
      var dom = $(
        '<div id="' +
          ...
          '">Hoshino</div>\
							<div id="' +
          hid +
          '"><br>Welcome To Sword Art Online</div>\
							<div id="' +
          mid +
          '">\
								<div id="' +
    ...
    panel.setPosX = function (value) {
      posX = value;
    };
    panel.setPosY = function (value) {
      posY = value;
    };
    ...
    panel.getPosX = function () {
      return posX;
    };
    panel.getPosY = function () {
      return posY;
    };
        ...
        .velocity(
          ...
          },
          {
            ...
            begin: function () {
              $("#" + raid)
                .css("left", 0)
                .velocity({ left: pwidth });
            },
            complete: function () {
              $("#" + baid).velocity({ height: baheight }, { duration: 100 });
              $("#" + did)
                .css({ height: 0 })
                .velocity({ height: dheight });
            },
          }
        );
      console.log(
...
var SAOMainMenu = {
  _new: function () {
    ...
    menu.getPos = function () {
      this.updateStartPos();
      return { x: mStartPosX, y: mStartPosY };
    };
    menu.getMenuName = function () {
      return _menu_name;
    };
    menu.getMaskName = function () {
      return _mask_name;
    };
    menu.getOverlayName = function () {
      return _overlay_name;
    };
    ...
    menu.createItem = function (name) {
      ...
      item_dom.click(function () {
        ...
        if (selected !== indexItem[item_id]) {
          ...
          if (
            panel_dom.css("display") === undefined ||
      ...
      item_dom.bind("mouseover", function () {
        var item_id = $(this).attr("id");
        if (indexItem[item_id] !== selected) {
          items[indexItem[item_id]].setSelected();
        }
      });
    ...
    menu.toggle = function () {
      ...
      if (
        overlay_dom.css("display") === undefined ||
...
Menu.createPanel();
// Menu.createItem('info', 'Info',  imageDIR, 'Welcome.');
// Menu.createItem('setting','Setting',imageDIR, 'blahblahblah...');
// Menu.createItem('plugin','Plugin',imageDIR, 'prprprprprpr...');
